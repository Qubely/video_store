...
use App\Http\Requests\Admin\MenuManagement\MainMenu\Crud\ValidateUpdateMainMenu;
...
use DB;
use Str;
class  MainMenuCrudRepository extends BaseRepository implements IMainMenuCrudRepository {

    use BaseTrait;
    public function __construct() {
        $this->LoadModels(['MainMenu']);
    }
    ...
    {
       return $this->getPageDefault(model: $this->MainMenu, id: $id);
    }
    ...
    {
        $model = MainMenu::query();
        $this->saveTractAction(
            $this->getTrackData(
                title: 'MainMenu was viewed by '.$request?->auth?->name.' at '.Carbon::now()->format('d M Y H:i:s A'),
                request: $request,
                onlyTitle: true
            )
        );
    ...
    public function store($request) : JsonResponse
    {
        ...
        try {
            MainMenu::create([
                ...
                'serial' => $this->facSrWc($this->MainMenu)
            ]);
            $response['extraData'] = ['inflate' => pxLang($request->lang,'','common.action_success') ];
            $this->saveTractAction($this->getTrackData(title: "MainMenu was created by ".$request?->auth?->name,request: $request));
            DB::commit();
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'MainMenu_store_error']), $e);
    ...
    {
        $row = MainMenu::find($id);
        if(empty($row)){
            return  $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-danger">'.pxLang($request->lang,'','common.no_resourse').'</span>']);
        }
        $rowRef = [...$row->toArray()];
        $row->fill([...$request->all(),'slug' => Str::kebab($request->name)]);
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateMainMenu())->rules($request,$row));
            if ($validator->fails()) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: " MainMenu ".$row?->name.' was updated by '.$request?->auth?->name,request: $request, row: $rowRef, type: 'to'));
                DB::commit();
                return $this->response(['type' => 'success','data' => $data]);
            } catch (\Exception $e) {
                DB::rollback();
                $this->saveError($this->getSystemError(['name'=>'MainMenu_update_error']), $e);
    ...
    {
        $i = MainMenu::whereIn('id',$request->ids)->select(['id','serial'])->get();;
        $dirty = [];
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "MainMenu list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'MainMenu_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = MainMenu::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "MainMenu list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
